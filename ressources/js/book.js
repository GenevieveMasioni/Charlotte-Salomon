$(function() {
  ...
  $('#conte').click(function(e){
    e.preventDefault();
    $('#mybook').booklet("gotopage", "5");
  });
  $('#roman').click(function(e){
    e.preventDefault();
  ...
  {    $("#results").click(function() {
    ...
    var cat15 = (!cat1 && !cat2 && !cat3 && !cat4 && !cat5 && !cat6 && !cat7 &&
      !cat8 && !cat9 && !cat10 && !cat11 && !cat12 && !cat13 && !cat14); var categories = [];
      ...
      var catStr = 'Les réponses suivantes sont incorrectes: ' + categories.join(', ') + '';
      $("#categorylist").text(catStr);
      $("#categorylist").show("slow");

      if (cat1 || !cat1) { $("#category1").show("slow"); };
      if (cat2 || !cat2) { $("#category2").show("slow"); };
      if (cat3 || !cat3) { $("#category3").show("slow"); };
      if (cat4 || !cat4) { $("#category4").show("slow"); };
      if (cat5 || !cat5) { $("#category5").show("slow"); };
      if (cat6 || !cat6) { $("#category6").show("slow"); };
      if (cat7 || !cat7) { $("#category7").show("slow"); };
      if (cat8 || !cat8) { $("#category8").show("slow"); };
      if (cat9 || !cat9) { $("#category9").show("slow"); };
      if (cat10 || !cat10) { $("#category10").show("slow"); };
      if (cat11 || !cat11) { $("#category11").show("slow"); };
      if (cat12 || !cat12) { $("#category11").show("slow"); };
      if (cat13 || !cat13) { $("#category13").show("slow"); };
      if (cat14 || !cat14) { $("#category14").show("slow"); };
